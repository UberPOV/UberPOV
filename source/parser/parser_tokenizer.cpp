    ...
    while (Token.Token_Id == END_OF_FILE_TOKEN)
    {
        ...
        switch (c)
        {
            ...
            case '_':
                Echo_ungetc(c);
                Read_Symbol ();
                if (!Parsing_Directive && (Inside_IdentFn == kIdentifierModeUndefined))
                {
                    IdentifierMode idMode;
                    switch (Token.Token_Id)
                    {
                        case LOCAL_TOKEN:       idMode = kIdentifierModeLocal;
                        case GLOBAL_TOKEN:      idMode = kIdentifierModeGlobal;
#if EXPERIMENTAL_UPOV_PERSISTENT
                        case PERSISTENT_TOKEN:  idMode = kIdentifierModePersistent;
#endif
                        default:                idMode = kIdentifierModeUndefined;
                    }
                    if (idMode != kIdentifierModeUndefined)
                    {
                        if (!Skip_Spaces())
...
void Parser::Read_Symbol()
{
    ...
    if (!Skipping)
    {
        ...
        switch (Inside_IdentFn)
        {
#if EXPERIMENTAL_UPOV_PERSISTENT
            case kIdentifierModePersistent: firstIndex = lastIndex = SYM_TABLE_PERSISTENT; break; // if inside "persistent()" pseudo-function, just test the persistent table
#endif
            case kIdentifierModeGlobal:     firstIndex = lastIndex = SYM_TABLE_GLOBAL;     break; // if inside "global()" pseudo-function, just test the global table
            case kIdentifierModeLocal:      firstIndex = lastIndex = Table_Index;          break; // if inside "local()" pseudo-function, just test the most local table
        }
