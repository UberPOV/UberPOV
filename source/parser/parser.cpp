...
#if EXPERIMENTAL_UPOV_PERSISTENT
    if(mExperimentalFlags.upovPersistent)       featureList.push_back("persistent data");
#endif
...
boost::posix_time::ptime Parser::Get_File_Time(const UCS2String& filename, unsigned int stype, bool err_flag)
{
    UCS2String fn(filename);
    UCS2String foundfile(backendSceneData->FindFile(GetPOVMSContext(), fn, stype));
    ...
    return backendSceneData->GetFileTime(GetPOVMSContext(), fn, foundfile.c_str(), stype);
}
